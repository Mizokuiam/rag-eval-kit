...
def my_dummy_llm_client(prompt: str) -> str:
    ...
    if "SUPPORTED or UNSUPPORTED" in prompt:
        if (
            "contradict" in prompt or "not found" in prompt
        ):  # Simulate finding unsupported content
            return random.choice(
                ["UNSUPPORTED", "The answer contains info not in context. UNSUPPORTED."]
            )
        else:
        ...
        if "capital of france" in prompt.lower() and "paris" in prompt.lower():
            ...
            return random.choice(
                ["IRRELEVANT", "The answer does not address the question. IRRELEVANT."]
            )
    else:
...
def main(
    dataset_path: Path = typer.Argument(
        ..., help="Path to the JSONL dataset file.", exists=True
    ),
):
    """
